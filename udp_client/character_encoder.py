...
class CharacterEncoder(object):
    ...
    def build_bit_pattern(self, character):
    ...
    def encode_bit_pattern(self, bit_pattern):
        binary = 0
        for x in range(len(bit_pattern)):
            for y in range(self.height):
                ...
                if bit_pattern[x][y]:
                    binary += 1
        return CharacterEncoder._binify(binary)

    def encode_character(self, character):
        bit_pattern = self.build_bit_pattern(character)
        return self.encode_bit_pattern(bit_pattern)
